...
class InventoryCostTestCase(ModuleTestCase):
    'Test Stock Inventory Cost'
    module = 'stock_inventory_cost'

    @with_transaction()
    def test_stock_inventory_cost(self):
        pool = Pool()
        ...
        with set_company(company):
            ...
            self.assertEqual(Decimal('100.0'), line_p1.cost_price)
            self.assertEqual(None, line_p1.total_cost)

            line_p1.quantity = 3
            line_p1.on_change_quantity()
            self.assertEqual(2.0, line_p1.diff_quantity)
            self.assertEqual(Decimal('100.0'), line_p1.cost_price)
            self.assertEqual(Decimal('200.0'), line_p1.total_cost)
            ...
            self.assertEqual(Decimal('70.0'), line_p2.cost_price)
            self.assertEqual(Decimal('0.0'), line_p2.total_cost)
            ...
            self.assertEqual(Decimal('100.0'), line_p1.cost_price)
            self.assertEqual(Decimal('100.0'), line_p1.total_cost)
            ...
            self.assertEqual(Decimal('70.0'), line_p2.cost_price)
            self.assertEqual(Decimal('-91.0'), line_p2.total_cost)

            # Inventory total cost
            line_p1.save()
            line_p2.save()
            inventory.save()
            self.assertEqual(Decimal('9.0'), inventory.total_cost)
    ...
    def _create_products(self):
        ...
        product = self._create_product(
            'Product', unit, Decimal('0.0'), Decimal('0.0'))

        kg, = ProductUom.search([('name', '=', 'Kilogram')])
        product2 = self._create_product(
            'Product 2', kg, Decimal('0.0'), Decimal('0.0'))
